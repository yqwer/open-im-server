...
func (u *UserMgo) Page(ctx context.Context, pagination pagination.Pagination) (count int64, users []*model.User, err error) {
	return mongoutil.FindPage[*model.User](ctx, u.coll, bson.M{"status": model.UserStatusNormal}, pagination)
}
...
func (u *UserMgo) GetAllUserID(ctx context.Context, pagination pagination.Pagination) (int64, []string, error) {
	return mongoutil.FindPage[string](ctx, u.coll, bson.M{"status": model.UserStatusNormal}, pagination, options.Find().SetProjection(bson.M{"_id": 0, "user_id": 1}))
}
...
func (u *UserMgo) CountTotal(ctx context.Context, before *time.Time) (count int64, err error) {
	if before == nil {
		return mongoutil.Count(ctx, u.coll, bson.M{"status": model.UserStatusNormal})
	}
	return mongoutil.Count(ctx, u.coll, bson.M{"status": model.UserStatusNormal, "create_time": bson.M{"$lt": before}})
}
...
func (u *UserMgo) SortQuery(ctx context.Context, userIDName map[string]string, asc bool) ([]*model.User, error) {
	...
	if len(attached) == 0 {
		filter := bson.M{"user_id": bson.M{"$in": userIDs}, "status": model.UserStatusNormal}

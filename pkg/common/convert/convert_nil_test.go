...
func getCommonUsersWithMissing(ctx context.Context, userIDs []string) (map[string]common_user.CommonUser, error) {
	users, err := getUsersWithMissing(ctx, userIDs)
	if err != nil {
		return nil, err
	}
	out := make(map[string]common_user.CommonUser, len(users))
...
func TestFriendRequestDB2PbNilSafeWhenUserArchived(t *testing.T) {
	...
	res, err := FriendRequestDB2Pb(context.Background(), reqs, getCommonUsersWithMissing)
	if err != nil {
